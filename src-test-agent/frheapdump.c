...
static void CheckJavaLangClassStamp(FollowReferencesArgs* args, jlong classTag, jlong javaLangClassTag) {
	ASSERT(classTag == 0L || StampIsClass(classTag), "classTag: %ld, javaLangClassTag: %ld", classTag,
			javaLangClassTag);

	if (args->javaLangClassTag == 0) {
		args->javaLangClassTag = javaLangClassTag;
	} else {
		ASSERT(javaLangClassTag == args->javaLangClassTag, "%ld != %ld", javaLangClassTag, args->javaLangClassTag);
	}
}

static jint heap_iteration_callback(jlong class_tag, jlong size, jlong* tag_ptr, jint length, void* user_data) {
...
static jint JNICALL HeapReferenceCallback(jvmtiHeapReferenceKind refkind, const jvmtiHeapReferenceInfo* refinfo,
	...
	if (isroot) {
		...
		if (refkind == JVMTI_HEAP_REFERENCE_STACK_LOCAL) {
			...
			const jvmtiHeapReferenceInfoStackLocal* label = &refinfo->stack_local;

			PrintStamp(args->out, label->thread_tag);

			fprintf(args->out, ":%ld:%d:%ld:%d", label->thread_id, label->depth, label->location, label->slot);
		...
		ASSERT(referrer_class_tag == 0L || StampIsClass(referrer_class_tag), "%ld", referrer_class_tag);
		ASSERT(referrer_tag_ptr != NULL, "");

		jlong referrer_tag = *referrer_tag_ptr;

		if (refkind == JVMTI_HEAP_REFERENCE_FIELD || refkind == JVMTI_HEAP_REFERENCE_STATIC_FIELD) {
			...
			if (refkind == JVMTI_HEAP_REFERENCE_CLASS) {
				ASSERT(referrer_class_tag == tag, "%ld != %ld", referrer_class_tag, tag);
				CheckJavaLangClassStamp(args, tag, class_tag);
			} else if (refkind == JVMTI_HEAP_REFERENCE_SUPERCLASS || refkind == JVMTI_HEAP_REFERENCE_INTERFACE) {
				CheckJavaLangClassStamp(args, referrer_tag, referrer_class_tag);
				CheckJavaLangClassStamp(args, tag, class_tag);
			}
		}
...
void NextHeapRequest(jvmtiEnv* jvmti) {
	...
	callbacks.heap_reference_callback = (jvmtiHeapReferenceCallback) &HeapReferenceCallback;
